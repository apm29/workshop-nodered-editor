<template>
  <div class="flex flex-col max-h-full h-full px-1 shadow ebox flex-nowrap">
    ...
    <DeviceEditor
      ...
      :table-name="DeviceTopographyTables.SWITCH"
      :table-dict="typeDict"
...
<script setup>
import { getPagedSwitch } from "~/api/device/paged";
...
const onPagination = async function ({ refresh } = {}) {
  ...
  loading.value = true;
  try {
    const res = await getPagedSwitch({
      pageNo: unref(page),
...
async function handleDelete(row) {
  await deleteDevice(row.id, DeviceTopographyTables.SWITCH);
  await onPagination();
}
</script>
<style lang="scss">
