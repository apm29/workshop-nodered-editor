<template>
  <div class="flex flex-col max-h-full h-full px-1 shadow ebox flex-nowrap">
    ...
    <DeviceEditor
      ...
      :table-name="DeviceTopographyTables.HUB"
      :table-dict="typeDict"
...
<script setup>
import { getPaged485Hub } from "~/api/device/paged";
...
const onPagination = async function ({ refresh } = {}) {
  ...
  loading.value = true;
  try {
    const res = await getPaged485Hub({
      pageNo: unref(page),
...
async function handleDelete(row) {
  await deleteDevice(row.id, DeviceTopographyTables.HUB);
  await onPagination();
}
</script>
<style lang="scss">
