<template>
  <div class="flex flex-col max-h-full h-full px-1 shadow ebox flex-nowrap">
    ...
    <DeviceEditor
      ...
      :table-name="DeviceTopographyTables.EBOX"
      :table-dict="typeDict"
...
import { getPagedElectricityBox } from "~/api/device/paged";
...
const onPagination = async function ({ refresh } = {}) {
  ...
  loading.value = true;
  try {
    const res = await getPagedElectricityBox({
      pageNo: unref(page),
...
async function handleDelete(row) {
  await deleteDevice(row.id, DeviceTopographyTables.EBOX);
  await onPagination();
}
</script>
<style lang="scss">
