<template>
  <div class="flex flex-col max-h-full h-full px-1 shadow ebox flex-nowrap">
    ...
    <DeviceEditor
      ...
      :table-name="DeviceTopographyTables.SENSOR"
      :table-dict="typeDict"
...
<script setup>
import { getPagedSensor } from "~/api/device/paged";
...
const onPagination = async function ({ refresh } = {}) {
  ...
  loading.value = true;
  try {
    const res = await getPagedSensor({
      pageNo: unref(page),
...
async function handleDelete(row) {
  await deleteDevice(row.id, DeviceTopographyTables.SENSOR);
  await onPagination();
}
</script>
<style lang="scss">
